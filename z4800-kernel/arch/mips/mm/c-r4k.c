...
static void __cpuinit r4k_blast_icache_page_indexed_setup(void)
{
	...
	if (ic_lsize == 0)
		...
		r4k_blast_icache_page_indexed = blast_icache128_page_indexed;
	else
		BUG();
}
...
static void __cpuinit probe_pcache(void)
{
	...
	case CPU_Z4800:
		icache_size = 8 << (((config & (0x1f << 13)) >> 13) + ((config & (0x1f << 18)) >> 18));
		c->icache.linesz = 8 << ((config & (0x1f << 13)) >> 13);
		c->icache.ways = ((config & (7 << 23)) >> 23) + 1;
		c->icache.waybit = 0;   /* doesn't matter */
		icache_size *= c->icache.ways;

		dcache_size = 4 << (((config & (0x1f << 0)) >> 0) + ((config & (0x1f << 5)) >> 5));
		c->dcache.linesz = 4 << ((config & (0x1f << 0)) >> 0);
		c->dcache.ways = ((config & (7 << 10)) >> 10) + 1;
		c->dcache.waybit = 0;   /* doesn't matter */
		dcache_size *= c->dcache.ways;
		break;
